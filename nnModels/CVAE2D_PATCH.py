...
class CVAE2D_PATCH_16(nn.Module):
    """
    Convolutional 2D variational autoencoder, used to test the method.
    :attr: beta: regularisation term of the variational autoencoder. Increasing gamma gives more importance to the KL
    divergence term in the loss.
    ...
    def __init__(self, latent_representation_size, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        super(CVAE2D_PATCH_16, self).__init__()
        nn.Module.__init__(self)
        self.beta = 5
        self.gamma = 100
        self.lr = 1e-4  # For epochs between MCMC steps
    ...
    def encoder(self, patch):
        ...
        h3 = F.gelu(self.bn3(self.conv3(h2)))
        # print("h3 shape =", h3.shape)
        mu = F.gelu(self.conv4_mu(h3))
        logvar = F.gelu(self.conv4_var(h3))
        # print("mu shape =", mu.shape)
        # print("logvar shape =", logvar.shape)
        return mu, logvar
    ...
    def to(self, *args, **kwargs):
        new_self = super(CVAE2D_PATCH_16, self).to(*args, **kwargs)
        device = next(self.parameters()).device
        self.device = device
        return new_self
...
class CVAE2D_PATCH_7(nn.Module):
    """
    Convolutional 2D variational autoencoder, used to test the method.
    :attr: beta: regularisation term of the variational autoencoder. Increasing gamma gives more importance to the KL
    divergence term in the loss.
    ...
    def __init__(self, latent_representation_size, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        super(CVAE2D_PATCH_7, self).__init__()
        nn.Module.__init__(self)
        self.beta = 5
        self.gamma = 100
        self.lr = 1e-4  # For epochs between MCMC steps
        ...
        # Encoder
        self.conv1 = nn.Conv2d(1, 8, kernel_size=3, stride=1)       # [8, 5, 5]
        self.conv2 = nn.Conv2d(8, 16, kernel_size=3, stride=1)      # [16, 3, 3]
        self.conv3_mu = nn.Conv2d(16, 16, kernel_size=3, stride=1)      # [16, 1, 1]
        self.conv3_var = nn.Conv2d(16, 16, kernel_size=3, stride=1)     # [16, 1, 1]

        self.bn1 = nn.BatchNorm2d(8)
        self.bn2 = nn.BatchNorm2d(16)
        self.bn3_mu = nn.BatchNorm2d(16)
        self.bn3_var = nn.BatchNorm2d(16)
        ...
        # Decoder
        # self.fc_decode = nn.Linear(latent_representation_size, 16) 
        self.deconv1 = nn.ConvTranspose2d(16, 16, kernel_size=3, stride=1)  # [16, 3, 3]
        self.deconv2 = nn.ConvTranspose2d(16, 8, kernel_size=3, stride=1)   # [8, 5, 5]
        self.deconv3 = nn.ConvTranspose2d(8, 1, kernel_size=3, stride=1)    # [1, 7 ,7]
    ...
    def encoder(self, patch):
        ...
        h2 = F.gelu(self.bn2(self.conv2(h1)))
        # print("h2 shape =", h2.shape)
        mu = F.gelu(self.conv3_mu(h2))
        logvar = F.gelu(self.conv3_var(h2))
        # print("mu shape =", mu.shape)
        # print("logvar shape =", logvar.shape)
        return mu, logvar
    ...
    def decoder(self, encoded):
        ...
        h7 = F.gelu(self.bn6(self.deconv2(h6))) 
        # print("Shape h7 =", h7.shape)
        h8 = F.sigmoid(self.deconv3(h7))
        # print("Shape h8 =", h8.shape)

        return h8

    def to(self, *args, **kwargs):
        new_self = super(CVAE2D_PATCH_7, self).to(*args, **kwargs)
        device = next(self.parameters()).device
        self.device = device
        return new_self
...
class CVAE2D_PATCH_32(nn.Module):
    """
    Convolutional 2D variational autoencoder, used to test the method.
    :attr: beta: regularisation term of the variational autoencoder. Increasing gamma gives more importance to the KL
    divergence term in the loss.
    ...
    def __init__(self, latent_representation_size=32, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        super(CVAE2D_PATCH_32, self).__init__()
        nn.Module.__init__(self)
        self.beta = 5
        self.gamma = 100
        self.lr = 1e-4  # For epochs between MCMC steps
        ...
        # Encoder
        self.conv1 = nn.Conv2d(1, 8, kernel_size=3, stride=1, padding=1)        # [8, 15, 15]
        self.conv2 = nn.Conv2d(8, 16, kernel_size=3, stride=2, padding=1)       # [16, 8, 8]
        self.conv3 = nn.Conv2d(16, 24, kernel_size=3, stride=2, padding=1)      # [24, 4, 4]
        self.conv4_mu = nn.Conv2d(24, 32, kernel_size=4, stride=1, padding=0)   # [32, 1 ,1]
        self.conv4_var = nn.Conv2d(24, 32, kernel_size=4, stride=1, padding=0)  # [32, 1 ,1]
        ...
        self.deconv4 = nn.ConvTranspose2d(8, 1, kernel_size=5, stride=1)     # [1, 15, 15]
    ...
    def to(self, *args, **kwargs):
        new_self = super(CVAE2D_PATCH_32, self).to(*args, **kwargs)
        device = next(self.parameters()).device
        self.device = device
        return new_self
...
class CVAE2D_PATCH_3latent32(nn.Module):
    """
    Convolutional 2D variational autoencoder, used to test the method.
    :attr: beta: regularisation term of the variational autoencoder. Increasing gamma gives more importance to the KL
    divergence term in the loss.
    ...
    def __init__(self, latent_representation_size, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        super(CVAE2D_PATCH_3latent32, self).__init__()
        nn.Module.__init__(self)
        self.beta = 5
        self.gamma = 100
        self.lr = 1e-4  # For epochs between MCMC steps
        ...
        self.conv2 = nn.Conv2d(8, 16, kernel_size=3, stride=2)       # [16, 3, 3]
        self.conv3_mu = nn.Conv2d(16, 32, kernel_size=3, stride=2)   # [32, 1 ,1]
        self.conv3_var = nn.Conv2d(16, 32, kernel_size=3, stride=2)  # [32, 1 ,1]
        self.bn1 = nn.BatchNorm2d(8)
        self.bn2 = nn.BatchNorm2d(16)

        # Decoder
        self.deconv1 = nn.ConvTranspose2d(32, 16, kernel_size=3, stride=2)   # [16, 3, 3]
        self.deconv2 = nn.ConvTranspose2d(16, 8, kernel_size=3, stride=2)    # [8, 7, 7]
        self.deconv3 = nn.ConvTranspose2d(8, 1, kernel_size=3, stride=2)     # [1, 15, 15]
        self.bn5 = nn.BatchNorm2d(16)
        self.bn6 = nn.BatchNorm2d(8)
    ...
    def to(self, *args, **kwargs):
        new_self = super(CVAE2D_PATCH_3latent32, self).to(*args, **kwargs)
        device = next(self.parameters()).device
        self.device = device
        return new_self
...
class CVAE2D_PATCH_64(nn.Module):
    ...
    def __init__(self, latent_representation_size=64, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        ...
        self.conv2 = nn.Conv2d(16, 32, kernel_size=3, stride=2, padding=1)      # [32, 4, 4]
        self.conv3_mu = nn.Conv2d(32, 64, kernel_size=4, stride=1)   # [64, 1 ,1]
        self.conv3_var = nn.Conv2d(32, 64, kernel_size=4, stride=1)  # [64, 1 ,1]
        self.bn1 = nn.BatchNorm2d(16)
        self.bn2 = nn.BatchNorm2d(32)

        # Decoder
        self.deconv1 = nn.ConvTranspose2d(64, 32, kernel_size=4, stride=1)   # [32, 4, 4]
        self.deconv2 = nn.ConvTranspose2d(32, 16, kernel_size=3, stride=2, padding=1, output_padding=1)    # [16, 8, 8]
        self.deconv3 = nn.ConvTranspose2d(16, 1, kernel_size=3, stride=2, padding=1)     # [1, 15, 15]
...
class CVAE2D_PATCH_4latent64(nn.Module):
    ...
    def __init__(self, latent_representation_size=64, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        ...
        self.conv1 = nn.Conv2d(1, 8, kernel_size=3, stride=1, padding=1)       # [8, 15, 15]
        self.conv2 = nn.Conv2d(8, 16, kernel_size=3, stride=2, padding=1)      # [16, 8, 8]
        self.conv3 = nn.Conv2d(16, 32, kernel_size=3, stride=2, padding=1)     # [32, 4, 4]
        self.conv4_mu = nn.Conv2d(32, 4, kernel_size=1, stride=1)   # [4, 4 ,4]
        self.conv4_var = nn.Conv2d(32, 4, kernel_size=1, stride=1)  # [4, 4 ,4]
        self.bn1 = nn.BatchNorm2d(8)
        self.bn2 = nn.BatchNorm2d(16)
        self.bn3 = nn.BatchNorm2d(32)

        # Decoder
        self.deconv1 = nn.ConvTranspose2d(4, 32, kernel_size=1, stride=1)   # [32, 4, 4]
        self.deconv2 = nn.ConvTranspose2d(32, 16, kernel_size=3, stride=2, padding=1, output_padding=1)    # [16, 7, 7]
        self.deconv3 = nn.ConvTranspose2d(16, 8, kernel_size=3, stride=2, padding=1)     # [8, 15, 15]
        self.deconv4 = nn.ConvTranspose2d(8, 1, kernel_size=3, stride=1, padding=1)     # [1, 15, 15]
        self.bn5 = nn.BatchNorm2d(32)
        self.bn6 = nn.BatchNorm2d(16)
        self.bn7 = nn.BatchNorm2d(8)
    ...
    def decoder(self, encoded):
        ...
        h8 = F.gelu(self.bn7(self.deconv3(h7))) 
        # print("Shape h8 =", h8.shape)
        h9 = F.sigmoid(self.deconv4(h8))
        # print("Shape h8 =", h9.shape)
    ...
    def forward(self, image):
        ...
        if self.training:
            ...
            encoded = mu
        reconstructed = self.decoder(encoded)
        return mu.view(-1, 64, 1, 1), logVar.view(-1, 64, 1, 1), reconstructed, encoded.view(-1, 64, 1, 1)
