...
from nnModels.losses import longitudinal_loss, spatial_2D_auto_encoder_loss, loss_bvae, loss_bvae2, \
    spatial_auto_encoder_loss
...
def train_with_crops_as_input(model, data_loader, longitudinal_estimator=None,
          longitudinal_estimator_settings=None, nb_epochs=100, nb_crops_per_image_per_epoch=5, lr=0.01,
    ...
    lr_scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=1, T_mult=2, eta_min=1e-7)
    nb_epochs_without_loss_improvement = 0
    losses = []

    iterator = tqdm(range(nb_crops_per_image_per_epoch), desc="Training", file=sys.stdout)
    for epoch in tqdm(range(nb_epochs)):
        nb_batch = 0
        model.training = True   # Unnecessary considering next line
        model.train()
        total_loss = []
        ...
        if longitudinal_estimator is not None:
            longitudinal_estimator, _ = fit_longitudinal_estimator_on_nn(data_loader, model, device,
