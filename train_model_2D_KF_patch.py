...
from nnModels.CVAE2D_ORIGINAL_PATCH import CVAE2D_ORIGINAL_PATCH
...
model = CVAE2D_ORIGINAL_PATCH(latent_representation_size)
model.gamma = gamma
model.beta = beta
...
output_path = f"training_plots/dataset_{temp_args.dataset}/VAE_patch_folds/{args.nnmodel_name}_{temp_args.dimension}_{temp_args.beta}_{temp_args.gamma}_{args.iterations}/"
...
if args.skip == "n":
    train_AE_kfold(CVAE2D_ORIGINAL_PATCH, folds_index, nb_epochs=300, device=device,
    ...
    best_fold = CV_VAE(CVAE2D_ORIGINAL_PATCH, folds_index, test_df, VAE_saving_path, temp_args.dataset, plot_save_path=output_path,
                    latent_dimension=latent_representation_size, gamma=gamma, beta=beta,
...
validation_dataset = LongitudinalDataset2D('data_csv/starmen_patch_validation_set.csv', read_image=open_npy, transform=transformations)
easy_dataset = LongitudinalDataset2D('data_csv/starmen_patch_train_set.csv', read_image=open_npy, transform=transformations)

data_loader = DataLoader(easy_dataset, batch_size=batch_size, num_workers=num_worker, shuffle=False, collate_fn=longitudinal_collate_2D)
validation_data_loader = DataLoader(validation_dataset, batch_size=batch_size, num_workers=num_worker, shuffle=False, collate_fn=longitudinal_collate_2D)
...
best_loss, lvae_losses = train_kfold(CVAE2D_ORIGINAL_PATCH, path_best_fold_model, folds_index, algo_settings, 
...
best_fold_LVAE = CV_LVAE(CVAE2D_ORIGINAL_PATCH, folds_index, test_df, nn_saving_path, longitudinal_saving_path, temp_args.dataset, plot_save_path=output_path)
...
best_fold_model = CVAE2D_ORIGINAL_PATCH(latent_representation_size)
